...
- **Cálculos**: se hacen en el servidor JavaScript, no en SQLite, para mantener lógica clara.
- **Consultas parametrizadas** (skill SQLite): siempre `pool.execute(sql, [params])` con `?`, nunca concatenar.
...
## Referencias
- **Skill SQLite**: transacciones, consultas parametrizadas, `insertId`.
