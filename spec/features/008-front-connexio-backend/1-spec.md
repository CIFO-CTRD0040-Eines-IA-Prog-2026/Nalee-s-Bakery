...
- `public/js/pedido.js`: al enviar un pedido, llama a `POST /api/orders` con los items del resumen. El catálogo de galletas se carga desde `GET /api/cookies` al iniciar.
...
- **Catálogo**: `GET /api/cookies` → pintar galletas con nombre según idioma.
